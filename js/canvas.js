...
window.flowerApp.render = function() {
    ...
    if (this.fills && this.fills.length > 0) {
        ...
        [...this.fixedStrokes, ...allStrokes].forEach(s => {
            ...
            fCtx.beginPath();
            fCtx.moveTo(s.points[0].x, s.points[0].y);
            for(let i=1; i<s.points.length; i++) fCtx.lineTo(s.points[i].x, s.points[i].y);
        ...
        // プレビューとしてメインキャンバスに合成
        this.ctx.globalAlpha = 1.0; // 以前は0.2だったが、薄すぎるため1.0に変更
    ...
    this.fixedStrokes.forEach(stroke => {
        ...
        this.ctx.moveTo(getScaledX(p0), getScaledY(p0));
        for (let i = 1; i < stroke.points.length; i++) {
            const p = stroke.points[i];
            this.ctx.lineTo(getScaledX(p), getScaledY(p));
        }
        if (stroke.fill) {
            this.ctx.closePath();
    ...
    allStrokes.forEach(stroke => {
        ...
        this.ctx.beginPath();
        const p0 = stroke.points[0];
        this.ctx.moveTo(this.offsetX + p0.x * this.scale, this.offsetY + p0.y * this.scale);
        for (let i = 1; i < stroke.points.length; i++) {
            const p = stroke.points[i];
            this.ctx.lineTo(this.offsetX + p.x * this.scale, this.offsetY + p.y * this.scale);
        }
...
window.flowerApp.eraseAt = function(pos) {
    const radius = this.lineWidth * 30;  // 60mm環境に合わせて調整 (5 * 6)
    let changed = false;
    let nextStrokes = [];
        ...
        stroke.points.forEach(p => {
            ...
            if (dist > radius) {
                ...
                if (currentPart.length > 1) {
                    nextStrokes.push({ ...stroke, points: currentPart });
                }
                currentPart = [];
                changed = true;
            }
